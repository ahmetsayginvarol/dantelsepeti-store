          ...
          <div className="block lg:hidden space-y-3">
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16">
                <p className="font-body text-lg text-ivory-500">Ürün bulunamadı</p>
              </div>
            )}
          </div>
                  ...
                  {filtered.map((product) => {
                    ...
                    return (
                      <tr key={product.id} className="border-b border-obsidian-600/40 hover:bg-obsidian-700/20 transition-colors">
                        ...
                        <td className="px-5 py-4 font-body text-base text-ivory-200 max-w-[180px] truncate">{product.title}</td>
      ...
      <label
        ...
        className={`flex flex-col items-center justify-center border-2 border-dashed transition-colors cursor-pointer p-6 text-center ${
          uploading
            ...
            : "border-obsidian-500 hover:border-gold-500/40 hover:bg-obsidian-700/30"
        }`}
      >
        {uploading ? (
          <>
          ...
          <>
            ...
            <p className="font-sans text-xs text-ivory-400 tracking-wider">
              Görsel seç veya buraya sürükle
            </p>
            <p className="font-sans text-[10px] text-ivory-600 mt-1">
              JPG, PNG, WEBP · Maks 5MB · Çoklu seçim
            </p>
          </>
        )}
      </label>
    ...
    <div className="fixed inset-0 modal-backdrop z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="luxury-card w-full max-w-2xl my-4">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gold-500/10 sticky top-0 bg-obsidian-800 z-10">
          ...
          <button
            onClick={onClose}
            className="text-ivory-500 hover:text-ivory-200 transition-colors p-1"
          >
            <X size={18} />
          </button>
        </div>
              ...
              <div className="flex gap-2 mb-3 flex-wrap">
                {uploadedImages.map((url, i) => (
                  <div key={i} className="relative w-16 h-16 bg-obsidian-700 overflow-hidden group">
                    ...
                    {i === 0 && (
                      <span className="absolute bottom-0 left-0 right-0 bg-gold-500/80 text-obsidian-900 text-[8px] text-center font-sans font-medium">
                        ANA
                      </span>
                    )}
                  </div>

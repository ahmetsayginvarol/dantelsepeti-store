      ...
      <div className="flex-1 min-w-0">
        <header className="h-16 bg-obsidian-800 border-b border-gold-500/10 flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            <span className="font-sans text-xs text-ivory-500 tracking-wider">admin@dantelsepeti.com</span>
        ...
        <main className="p-6 lg:p-8">
          ...
          <div className="luxury-card overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gold-500/10">
              ...
              <Link href="/admin/products" className="font-sans text-xs tracking-wider text-gold-400 hover:text-gold-300 transition-colors uppercase">
                Tümünü Gör →
              </Link>
            </div>
                ...
                <thead>
                  <tr className="border-b border-gold-500/5">
                    {["SKU", "Ürün Adı", "Fiyat", "Kategori", "Durum"].map((h) => (
                      <th key={h} className="text-left px-6 py-3 font-sans text-[10px] tracking-[0.2em] uppercase text-ivory-600">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {demoProducts.slice(0, 6).map((product) => {
                    ...
                    return (
                      <tr key={product.id} className="border-b border-obsidian-600/50 hover:bg-obsidian-700/30 transition-colors">
                        <td className="px-6 py-4 font-sans text-xs text-gold-500/70 tracking-wider">{product.sku}</td>
                        <td className="px-6 py-4 font-body text-base text-ivory-200">{product.title}</td>
                        <td className="px-6 py-4 font-sans text-sm text-ivory-300">{formatPrice(product.price)}</td>
                        <td className="px-6 py-4 font-sans text-xs text-ivory-500 tracking-wider">{category?.name || "—"}</td>
                        <td className="px-6 py-4">
                          <span className={`font-sans text-[10px] tracking-wider uppercase px-2 py-1 ${product.is_available ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
